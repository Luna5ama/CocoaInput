...
public class TinyConfig {
    ...
    public static void init(String modid, Path apath, Class<?> config) {
        ...
        for (Field field : config.getFields()) {
            Entry e;
            try {e = field.getAnnotation(Entry.class);} catch (Exception ignored) {continue;}
            if (e == null) {continue;}
        ...
        try {gson.fromJson(Files.newBufferedReader(path), config);} catch (Exception e) {write();}
    ...
    private static void textField(
        ...
        info.widget = (BiFunction<EditBox, Button, Predicate<String>>) (t, b) -> s -> {
            ...
            if (!(isNumber && s.isEmpty()) && !s.equals("-") && !s.equals(".")) {
                ...
                info.error = inLimits ? null : new AbstractMap.SimpleEntry<>(
                    t,
            ...
            if (inLimits) {info.value = isNumber ? value : s;}
            ...
            Button done = addRenderableWidget(
                Button.builder(
                        Component.translatable("gui.done"), (button) -> {
                            for (EntryInfo info : entries) {
                                try {info.field.set(null, info.value);} catch (IllegalAccessException ignore) {}
                            }
                            write();
                ...
                if (info.widget instanceof Map.Entry) {
                    ...
                    addRenderableWidget(
                        Button.builder(widget.getValue().apply(info.value), widget.getKey())
                            .bounds(width - 85, y, info.width, 20)
                            .build()
                    );
                } else {
                    ...
                    Predicate<String> processor = ((BiFunction<EditBox, Button, Predicate<String>>) info.widget).apply(
                        widget,
                        done
                    );
            ...
            for (EntryInfo info : entries) {
                ...
                if (info.error != null && info.error.getKey().isMouseOver(mouseX,mouseY))
                    ...
                    if (info.dynamicTooltip != null) {
                        try {
                            renderComponentTooltip(matrices, (List<IComponent>) info.dynamicTooltip.invoke(null, entries), mouseX, mouseY);
                            y += 30;
                            continue;
